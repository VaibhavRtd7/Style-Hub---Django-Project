...
def index(request):
    wishlist_items = WishList.objects.filter(user=request.session.get('user_id'))
...
def registerUser(request):
    if request.method == "POST":
        ...
        user = User(first_name=firstname, last_name=lastname, email=email, password=hashed_password,phone_number=phone_number,date_registered=datetime.today(),user_type=user_type)
        user.save()
...
def ProductView(request):
    ...
    wishlistItem = WishList.objects.filter(user_id=user_id).values_list('product__id', flat=True)
...
def cart(request):
    ...
    total_amount = sum([item.quantity * item.item.amount for item in order_item])
...
def add_to_cart(request, product_id):
    ...
    order_item, created = OrderItem.objects.get_or_create(item=product, ordered=False, user=user)
...
def remove_from_cart(request, order_item_id):
    ...
    order_item = get_object_or_404(OrderItem, id=order_item_id, user=request.session.get('user_id'), ordered=False)
    ...
    order = Order.objects.filter(user=request.session.get('user_id'), date_shipped=None).first()
...
def checkout(request):
    if not request.session.get('user_id'):
        return redirect('/login')
    address = Address.objects.filter(user=request.session.get('user_id')).first()
    context = {
        'countries': countries,
        'user': address
    }
...
def contact(request):
    if request.method == 'POST':
        ...
        obj = Contact(name=name, subject=subject, email=email, message=send_message)
        obj.save()
